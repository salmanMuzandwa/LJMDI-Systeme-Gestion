...
import { ArrowBack, Edit, Delete, Print, VolunteerActivism, Handshake, CardGiftcard, Campaign } from '@mui/icons-material';
...
const VoirContribution = () => {
    ...
    useEffect(() => {
        const fetchContribution = async () => {
            ...
            try {
                ...
                if (data?.id_membre) {
                    try {
                        const memberResponse = await axios.get(`/api/membres/${data.id_membre}`, {
                            headers: {
                                'Authorization': `Bearer ${token}`
                            }
                        });
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (member?.id_membre) {
                await axios.put(`/api/membres/${member.id_membre}`, {
                    nom: editValues.nom,
    ...
    const getTypeInfo = (type) => {
        const types = {
            ...
            'speciale': { label: 'Spéciale', icon: <Campaign />, color: '#f9ca24' }
        };
    ...
    if (error || !contribution) {
        return (
            <Container sx={{ mt: 4 }}>
                <Alert severity="error">{error || "Contribution non trouvée"}</Alert>
                <Button
                    startIcon={<ArrowBack />}
